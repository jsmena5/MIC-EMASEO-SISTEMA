...
const VALID_TOKEN   = makeJwt({ id: '1', nombre: 'Admin', rol: 'admin', exp: NOW + 3600 })
const EXPIRED_TOKEN = makeJwt({ id: '2', nombre: 'Bob',   rol: 'admin', exp: NOW - 3600 })
const NO_EXP_TOKEN  = makeJwt({ nombre: 'X', rol: 'admin' })
...
describe('isTokenExpired', () => {
  ...
  it('applies skewSeconds: token expiring in 30s is expired with 60s skew', () => {
    const token = makeJwt({ nombre: 'X', rol: 'admin', exp: NOW + 30 })
...
describe('hasAllowedRole', () => {
  const user: AuthUser = { nombre: 'Admin', rol: 'admin' }

  it('returns true when role is in the allowed list', () => {
    expect(hasAllowedRole(user, ['admin', 'superadmin'])).toBe(true)
  })

  it('returns false when role is not in the allowed list', () => {
    expect(hasAllowedRole(user, ['supervisor', 'operador'])).toBe(false)
  })
