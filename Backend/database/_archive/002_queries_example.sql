-- ============================================================================
-- MIC-EMASEO SISTEMA — Consultas de Ejemplo
-- ============================================================================
-- Estas consultas representan las operaciones mas frecuentes del sistema.
-- Estan optimizadas por los indices definidos en 001_schema.sql.
-- ============================================================================
...
SELECT
    ...
    ar.nivel_acumulacion
FROM incidents.incidents i
JOIN auth.users u ON u.id = i.reportado_por
LEFT JOIN ai.analysis_results ar ON ar.incident_id = i.id
...
-- Opcion A: Usando zona_id (asignada por trigger)
...
-- Opcion B: Usando contencion espacial directa (ST_Contains)
-- Util si se necesita consultar por geometria sin depender del trigger
...
LEFT JOIN ai.analysis_results ar ON ar.incident_id = i.id
LEFT JOIN auth.users sup ON sup.id = z.supervisor_id
WHERE z.activa = TRUE
GROUP BY z.id, z.codigo, z.nombre, sup.nombre, sup.apellido
...
SELECT
    ...
    sup.nombre || ' ' || sup.apellido AS asignado_por
FROM incidents.assignments a
JOIN incidents.incidents i ON i.id = a.incident_id
JOIN auth.users sup ON sup.id = a.asignado_por
...
-- Retorna centroides agrupados en celdas hexagonales.
...
WHERE i.created_at >= NOW() - INTERVAL '30 days'
  AND i.estado <> 'RECHAZADA'
GROUP BY
    -- Agrupacion en celdas de ~100m (truncar coordenadas a 3 decimales)
